<div class="card sm:w-72 bg-base-200 shadow-xl mb-8 h-fit" x-data>
    ...
    <div class="card-body items-center text-center">
        ...
        @endif
        {{-- DESC TIDAK DITAMPILKAN DULU KARENA MENGGUNAKAN WYSIWYG STYLE NYA KETIMPA (WARNA TEXT NYA HITAM JADI SAAT TEMA DARK GA KELIATAN) --}}
        {{-- <p>{!! Str::limit($product->description, 50, '...') !!}</p> --}}

        @can('is-member')
            {{-- HARGA PRODUK HANYA BISA DILIHAT OLEH MEMBER yang sudah verif --}}
            @if (auth()->user()->hasVerifiedEmail())
                <div>
                            ...
                            <div class="flex flex-col">
                                <div class="flex">
                                    <span class="line-through">
                                        ...
                                        <x-client.format-rp :value="$product->product_variant[count($product->product_variant) - 1]
                                            ->price" />
                                    </span>
                                ...
                                <div>
                                    <x-client.format-rp :value="$product->product_variant[0]->price -
                                        ($product->product_variant[0]->price * $product->promo->discount) / 100" />
                                    &nbsp;-&nbsp;
                                    <x-client.format-rp :value="$product->product_variant[count($product->product_variant) - 1]->price -
                                        ($product->product_variant[count($product->product_variant) - 1]->price *
                    ...
                    @else
                        ...
                        @if (isset($product->promo) && $product->promo->active)
                            <span class="line-through">
                                <span>{{ 'Rp. ' . number_format($product->product_variant->first()->price, 0, ',', '.') }}</span>
                            </span>
                            <span class="text-xs align-top ml-2">-{{ $product->promo->discount }}%</span>
                            <br />
                            <span>{{ 'Rp. ' .
                                number_format(
                                    $product->product_variant->first()->price -
                                        ($product->product_variant->first()->price * $product->promo->discount) / 100,
                                    0,
                                    ',',
                                    '.',
                                ) }}</span>
                ...
                <div class="card-actions">
                    <div class="flex gap-2">
                        <input type="number" name="quantity" x-ref="quantity"
                            class="my-input px-1 w-16 dark:border-b-white " min="1" value="1">
            ...
            @else
                <div class="tooltip tooltip-bottom before:w-40" data-tip="Verifikasi email untuk menambahkan ke keranjang">
                    <a href="{{ route('verification.notice') }}" class="btn btn-circle btn-primary">
                        <i class="fa fa-cart-shopping"></i>
                    </a>
                </div>
            @endif
