<x-client-layout>
    <div class="cart-body p-2" x-data>
        ...
        @if ($errors->has('no_address'))
        <div x-transition:leave.duration.500ms class="toast toast-top toast-end mt-24 z-50" x-data="{show: true}" x-init="setTimeout(() => show = false, 5000)" x-show="show">
            <div role="alert" class="alert alert-error mb-4">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 shrink-0 stroke-current" fill="none"
                    viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <span>{{$errors->first('no_address')}}</span>
            </div>
        </div>
        @endif
    </div>
