    ...
    <div class="notice mb-6" x-data="{
        ...
        <div role="alert" class="alert bg-primary text-primary-content py-2">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                class="stroke-green-300 dark:stroke-black animate-pulse shrink-0 w-6 h-6">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>
            </svg>
            <span x-text="noticeMessage"></span>
            <div class="flex gap-1">
                <a href="{{ $notice->link }}" class="btn btn-sm bg-green-600 dark:bg-secondary text-green-200 dark:text-primary border-green-600 dark:border-primary-content hover:text-green-700"
            ...
            <div class="!w-60 sm:!w-80 swiper heroSwiper">
                <div class="swiper-wrapper">
                    ...
                    <div class="swiper-slide rounded-box mask mask-squircle">
                        <img src="{{ Storage::url($product->product_pictures->first()->path) }}" class="w-full"
                            alt="product images" />
                    </div>
                    @endif
                    @endforeach
                    ...
                    <a href="{{ route('client.product', $promo_item->product->slug) }}">
                        <div class="absolute inset-0 flex items-center justify-center text-3xl">
                            <div
                                ...
                                @auth
                                <x-client.format-rp value="{{ $promo_item->product->product_variant[0]->price }}" />
                                @endauth
                                @guest
                        ...
                        <img src="{{ Storage::url($promo_item->product->product_pictures->first()->path) }}" />
                        @endif
                    </a>
                </div>
                @endforeach
                ...
                <div class="swiper-wrapper lg:mx-24">
                    ...
                    <div class="swiper-slide [&>img]:h-full [&>img]:w-full">
                        ...
                        <a href="{{ route('client.product', $auction_item->product->slug) }}">
                            ...
                            <img src="{{ Storage::url($auction_item->product->product_pictures->first()->path) }}" />
                            @endif
                        </a>
                    </div>
                    @endforeach
